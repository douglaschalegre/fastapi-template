    ...
    def builder():
        ...
        for api in external_apis:
            if api['open_api_path'] is not None:
                raw_docs = request(
                    method='GET', url=f'{api["url"]}{api["open_api_path"]}', timeout=300
                )
                if raw_docs.status_code == 200:
                    ...
                    openapi_schema['tags'] = openapi_schema.get('tags', []) + [
                        dict(name=f'[PROXY] {api["name"]}', description=api['description'])
                    ]

                else:
                    print(f'Error when get OpenAPI from {api["name"]} with url {api["url"]}')
